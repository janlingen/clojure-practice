(ns problems.medium.141)

(comment "Tricky card games")


(comment "In trick-taking card games such as bridge, spades,
...
(assert (and (let [notrump (solution nil)] (and (= {:suit :club, :rank 9} (notrump [{:suit :club, :rank 4} {:suit :club, :rank 9}])) (= {:suit :spade, :rank 2} (notrump [{:suit :spade, :rank 2} {:suit :club, :rank 10}]))))
             (= {:suit :club, :rank 10} ((solution :club) [{:suit :spade, :rank 2} {:suit :club, :rank 10}]))
             (= {:suit :heart, :rank 8} ((solution :heart) [{:suit :heart, :rank 6} {:suit :heart, :rank 8} {:suit :diamond, :rank 10} {:suit :heart, :rank 4}]))))
